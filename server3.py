...
class VideoStreamingServer:
    def __init__(self, host='10.177.60.18', video_port=8888, control_port=8889, video_file_path=None):
        self.host = host
    ...
    def handle_client_control(self, client_socket, addr):
        ...
        try:
            # Register client with default resolution when they first connect
            self.clients[addr] = '480p'  # Default resolution
                ...
                try:
                    ...
                    if message['type'] == 'resolution_request':
                        ...
                        if requested_resolution in self.resolutions:
                            old_resolution = self.clients.get(addr, '480p')
                            self.clients[addr] = requested_resolution
                            print(f"Client {addr} resolution changed: {old_resolution} -> {requested_resolution}")
    ...
    def start_video_streaming(self):
        ...
        while self.is_streaming:
            try:
                ...
                # Debug: Log client status every 30 frames (1 second at 30fps)
                if sequence_number % 30 == 0:
                    print(f"[DEBUG] Frame #{sequence_number}: {len(self.clients)} clients, resolutions: {active_resolutions}")
    ...
    def broadcast_packet(self, packet, resolution):
        ...
        try:
            client_address = ('10.177.60.65', 8890)  # Client listening port
