...
class VideoStreamingClient:
    def __init__(self, server_host='127.0.0.1', video_port=8890, control_port=8889):
        ...
        print(f"[LOCALHOST] Client initialized for server {server_host}:{video_port}")
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            print(f"[LOCALHOST] Video socket bound to port {self.video_port}")
            
            # Connect control socket
            self.control_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.control_socket.connect((self.server_host, self.control_port))
            print(f"[LOCALHOST] Connected to control server at {self.server_host}:{self.control_port}")
            print(f"[TCP] Client successfully connected to TCP control port {self.control_port}")
            
            # Send initial registration message to activate streaming
            registration_message = {
                'type': 'client_registration',
                'video_port': self.video_port,
                'timestamp': time.time()
            }
            self.control_socket.send(json.dumps(registration_message).encode())
            print(f"[LOCALHOST] Sent registration message to server")
            ...
            try:
                ...
                if ack['type'] == 'registration_ack' and ack['status'] == 'success':
                    print(f"[LOCALHOST] Registration acknowledged by server - streaming should start!")
                else:
                    print(f"[LOCALHOST] Unexpected registration response: {ack}")
            except Exception as e:
                print(f"[LOCALHOST] Error receiving registration ack: {e}")
        ...
        if self.control_socket:
            try:
                ...
                if ack['type'] == 'resolution_ack':
                    print(f"[LOCALHOST] Resolution change acknowledged: {ack['resolution']}")
                    return True
    ...
    def start_streaming(self):
        ...
        try:
            print("\n🎥 Starting video streaming client (Localhost)...")
    ...
    def receive_video(self):
        ...
        cv2.namedWindow('Video Stream (Localhost)', cv2.WINDOW_NORMAL)  # Allow manual resizing
        cv2.resizeWindow('Video Stream (Localhost)', 1920, 1080)  # Set to 1080p size
        cv2.moveWindow('Video Stream (Localhost)', 0, 0)  # Position window at top-left corner
        ...
        while self.is_running:
            try:
                ...
                if frame_data is not None:
                    ...
                    if frame_count % 50 == 0:
                        print(f"[DEBUG] Received frame #{frame_count}, resolution: {resolution}, data size: {len(frame_data)} bytes")
                    ...
                    if frame is not None:
                        ...
                        cv2.imshow('Video Stream (Localhost)', display_frame)
                        
                        # Check for quit command
                        key = cv2.waitKey(1) & 0xFF
                        if key == ord('q'):
                            print("[LOCALHOST] User pressed 'q' - stopping client...")
                            self.is_running = False
                            break
                        elif key == ord('f'):
                            # Toggle fullscreen
                            cv2.setWindowProperty('Video Stream (Localhost)', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                        elif key == ord('w'):
                            # Return to windowed mode
                            cv2.setWindowProperty('Video Stream (Localhost)', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_AUTOSIZE)
                    else:
                        print(f"[DEBUG] Failed to decode frame data, size: {len(frame_data)} bytes")
                else:
                    print("[DEBUG] Failed to parse packet")
                
            except Exception as e:
                if self.is_running:
                    print(f"Error receiving video: {e}")
        
        cv2.destroyWindow('Video Stream (Localhost)')
    ...
    def add_metrics_overlay(self, frame, actual_resolution=None, window_size=None):
        ...
        # 3. Localhost indicator
        cv2.putText(frame, "Mode: LOCALHOST", 
    ...
    def cleanup(self):
        ...
        print("🧹 [LOCALHOST] Client cleanup completed")
